package com.example.realtimeimageprocessing.camera;
...
import com.example.realtimeimageprocessing.R;
...
public class CameraFragment extends Fragment {
    ...
    public static CameraFragment newInstance(
            ...
            final Size inputSize,
            final String cameraId) {
        return new CameraFragment(callback, imageAvailableListener, inputSize,
                cameraId);
    }
        ...
        try {
            if (!cameraOpenCloseLock.tryAcquire(2500, TimeUnit.MICROSECONDS)) {
                Toast.makeText(getContext(),
                        "Time out waiting to lock camera opening.", Toast.LENGTH_LONG).show();
                activity.finish();
            } else {
    ...
    protected Size chooseOptimalSize(final Size[] choice, final int width, final int height) {
        final int minSize = Math.min(width, height);
        final Size desiredSize = new Size(width, height);

        final List<Size> bigEnough = new ArrayList<Size>();
        final List<Size> tooSmall = new ArrayList<Size>();
        for (final Size option : choice) {
        ...
        if (bigEnough.size() > 0) {
            ...
            return Collections.max(tooSmall, new CompareSizeByArea());
        }
    }
